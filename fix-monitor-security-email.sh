...
# Fix für E-Mail-Flut - Direkt auf Server ausführen
# ssh root@194.55.13.15
# cd /var/www/omnireflect
# ./fix-monitor-security-email.sh
...
echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  E-Mail-Flut Fix - auto-security-response.sh${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
echo ""
...
if [ -f "/usr/local/bin/auto-security-response.sh" ]; then
    ...
    echo -e "${YELLOW}⚠️  auto-security-response.sh nicht gefunden, erstelle neu${NC}"
fi

# Neue Version mit Deduplizierung erstellen
echo ""
...
echo ""
echo -e "${BLUE}📋 Prüfe Cron-Job:${NC}"
crontab -l | grep auto-security-response || echo "  (Kein Cron-Job gefunden)"
echo ""
